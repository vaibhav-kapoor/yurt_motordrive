...
int _write(int file, char *ptr, int len)
{
        ...
        if (file == 1) {
			if (len) {
				while (usbd_ep_write_packet(usbd_dev_, 0x82, (uint8_t *)ptr, len) == 0)
					;
			}        		
                if (ret < 0)
                        ret = -ret;
...
void usbacm_init(void)
{
	...
	usbd_dev_ = usbd_init(&otgfs_usb_driver, &dev, &config, usb_strings, 3, usbd_control_buffer, sizeof(usbd_control_buffer));
	usbd_register_set_config_callback(usbd_dev_, cdcacm_set_config);
